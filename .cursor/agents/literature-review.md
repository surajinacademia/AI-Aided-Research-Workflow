...
1. Parse the query for key concepts and search terms.
2. Search in order: local literature → project documentation → Zotero (MCP) → web if gaps remain.
3. Synthesize and return a structured answer with citations. Do not stop at the first hit; search systematically.
